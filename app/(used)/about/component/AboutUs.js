    ...
    const fetchData = async () => {
      try {
        ...
        const response = {
          ...
          alumni: await fetch(
            `${process.env.NEXT_PUBLIC_API_URL}/public/alumni?isShown=true`,
            { cache: "no-store" }
          ).then((res) => res.json()),
          dewan: await fetch(
        ...
        if (response.alumni?.length > 0) {
          response.alumni = sortingItem(response.alumni);
        }
                ...
                <div className="col-md-6 ">
                  <div className="apartment-imgs">
                    ...
                    <div className="apartment-img2">
                      <img
                        className="tw-w-[30rem] tw-h-[40rem]"
                        src={data?.aboutUs?.data?.image}
                        alt=""
                      />
                    </div>
          ...
          <div className="tw-bg-[#3e90ba] tw-py-5 page-bg">
            <div className="container">
              ...
              <div className="row">
                <AlumniSlider
                  data={data?.alumni}
                  className="tw-scale-100 tw-w-full group-hover:tw-scale-110 android:tw-scale-[0.5] android:group-hover:tw-scale-75"
                  useIcon={false}
                />
              </div>
            </div>
